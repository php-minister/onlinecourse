...
<?php $this->load->view('teacher/menu',array('active_menu'=>'gradebook'))?>
<script type="text/javascript">
    ...
    $('document').ready(function(){
        $("#group_id").change(function(){
             $("#student_page").val(1);
             <?php if ((isset($semester['name'])) AND ($semester['is_active']=='1')){?>
             ajax_query('teacher/get_gradebook_subjects/'+$(this).val(),'subjects_area');
             <?php }?>
         });
  ...
  function remove_assigment(set_id)
  {
      ...
      {
          ajax_query('teacher/delete_gradebook_set/'+set_id,'',function(){
  ...
  function save_scores()
  {
      ...
      {
          block_screen();
          $.post('teacher/save_gradebook_scores',{scores:JSON.stringify(changes)},function(data){
                 if (data.error)
                 {
...
<header>
    ...
    <?php if ((isset($semester['name'])) AND ($semester['is_active']=='1')){?>
    <small>for <?= ($semester['name'])?('<span title="'.date('d M Y',strtotime($semester['start_date'])).' - '.date('d M Y',strtotime($semester['end_date'])).'">'.$semester['name'].'</span><br/>'):('<i>'.date('d M Y',strtotime($semester['start_date'])).' - '.date('d M Y',strtotime($semester['end_date'])).'</i>')?></small>
    <?php }?>
    </h2>
    ...
    <div class="alert alert-error"><?= $this->lang->line('make_sure_admin_made_previous_semester_completed')?></div>
    <?php }?>
...
<section>
    <article>
    <div class="span3">
        <form action="teacher/get_gradebook_students" id="students_form" method="POST">
            ...
            <div id="subjects_area">
                <?php $this->load->view('teacher/gradebook_subjects');?>
            </div>
        </form>
        <br/>
        <?php if ((isset($semester['name'])) AND ($semester['is_active']=='1')){?>
        <a href="teacher/gradebook#students_list" class="btn btn-info" onclick="submit_form('#students_form')"><?= $this->lang->line('show_students')?></a>
        <?php }?>
