<div class="modal show">
    <div class="modal-header">
        ...
        <h3><?= $this->lang->line('edit_group')?></h3>
    </div>
    <div class="modal-body">
        ...
        <form action="students_groups/save_group" method="POST" id="group_form">
            ...
            <div class="control-group">
                ...
                <input type="text" name="name" id="name" maxlength="50" class="required" value="<?= $group['group_name']?>">
            </div>
            <div class="control-group">
                <label for="grade"><?= $this->lang->line('grade')?></label>
                <select name="grade" id="grade">
                    <?php foreach($grades as $grade){?>
                    <option value="<?= $grade['grade_id']?>" <?= ($grade['grade_id']==$group['grade_id']?'selected="selected"':'')?>><?= $grade['name']?></option>
                    <?php }?>
                </select>
            </div>
        </form>
