...
<script>
    $('document').ready(function(){
        $("#date").datepicker({
            set_date:function(){
                get_date_grades($("#date").val(),'admin');
            },
            end:'<?= date('Y-m-d')?>'
        });
    })
...
<section>
    <article>
        <div class="span3" style="margin-bottom: 10px;">
            ...
            <form action="attendance/show_students" id="show_students" method="POST">
                <div class="control-group">
                    <label for="date"><?= $this->lang->line('date')?>:<sup class="mandatory">*</sup></label>
                    <input type="text" name="date" id="date" value="<?= date('m/d/Y')?>" class="required" onchange="get_date_grades($('#date').val(),'admin');">
                </div>
            ...
            <br/>
            <a  href="attendance#students_list" class="btn btn-info" onclick="submit_form('#show_students')"><?= $this->lang->line('show_students')?></a>
        </div>
