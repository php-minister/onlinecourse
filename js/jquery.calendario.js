					...
					if ( day <= monthLength && ( i > 0 || j >= p ) ) {
						...
						if( dayData ) {
                            ...
                            for(var index in dayData)
                            {
                                ...
                                content +='<span title="'+start_date+' - '+end_date+', '+dayData[index].title.replace('{|}', ' ') + '" '+(dayData[index].allow_edit==0?' class="event-disabled"':'')+' id="'+dayData[index].id+'" class="trigger_class">' + dayData[index].start +' '+dayData[index].title.split('{|}')[0]+'</span>';
                            }
						}
							
						if( new_loop!= 0)	{
						 	//content+= '<span class="">'+new_loop+'</span>';
						}
        ...
        getDate : function(date)
        {
            ...
            for(var index in this.caldata[date])
            {
                ...
                dates +='<li class="event '+(i%2==0?'even':'odd')+'" id="div_'+index+'_'+i+'">'+
                       '<div class="time">'+this.caldata[date][index].start.replace(/:[0-9]+$/gi,'')+' - '+this.caldata[date][index].end.replace(/:[0-9]+$/gi,'')+'</div>'+
                       '<div class="title">';                
                 ...
                 dates +='</div>'+
                         ...
                         '<p class="description">'+this.caldata[date][index].title.replace('{|}', ' ')+'</p>'+
                         '</li>';
            }
